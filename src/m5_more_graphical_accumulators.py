...
def draw_circles_from_rectangle(m, n, rectangle, window):
    ...
    sideex = abs(rectangle.corner_1.x - rectangle.corner_2.x)
    sideey = abs(rectangle.corner_1.y - rectangle.corner_2.y)

    for k in range(m):
        point = rg.Point(rectangle.get_center().x - sideex / 2 - (2 * k + 1) * sideey / 2, rectangle.get_center().y)
        circle = rg.Circle(point, sideey / 2)
        circle.fill_color = rectangle.fill_color
        circle.attach_to(window)

    for j in range(n):
        point1 = rg.Point(rectangle.get_center().x, rectangle.get_center().y - sideey / 2 - (2 * j + 1) * sideex / 2)
        circle = rg.Circle(point1, sideex / 2)
...
def draw_lines_from_rectangles(rectangle1, rectangle2, n, window):
    ...
    x = abs(rectangle1.get_center().x - rectangle1.corner_1.x)
    y = abs(rectangle1.get_center().y - rectangle1.corner_1.y)

    for k in range(n):
        point1 = rg.Point(rectangle1.get_center().x - k * x, rectangle1.get_center().y + k * y)
        point2 = rg.Point(rectangle2.get_center().x - k * x, rectangle2.get_center().y + k * y)
